...
- [x] T051 [US3] [3] `commands/linear-status.md` — AI-invoked: surfaces current sync status for the current consumer repo and all its specs (lifecycle phase, worktree authority, last sync timestamp). ✓ 2026-05-28 — landed in commit `0f716c1`. Implementation in `src/status.sh` + bats in `tests/integration/us3-status-staleness.bats`. Read-only — pure `graphql::query`, zero mutations.
- [x] T052 [US3] [5] `commands/linear-pull.md` — AI-invoked: queries Linear for the current state of all spec Issues in this repo's Project and prints them locally (read-only operation; never mutates). ✓ 2026-05-28 — implementation in `src/pull.sh` (866 lines) + bats in `tests/integration/us3-pull-cross-repo.bats` (4 scenarios). `--repo` (default) and `--workspace-wide` scopes; `--phase` filter; `--human` and `--json` output. Uses `IssueFilter.and` with `labels.name startsWith "speckit-spec:"` for the cross-repo lookup.
- [x] T053 [P] [US3] [1] `src/reconcile.sh`: ensure `--all` mode handles repo-level filtering — when invoked without `--spec`, iterate every `specs/NNN-feature/` and reconcile each, scoped to the repo's Project UUID from config (no cross-Project bleed) ✓ 2026-05-28 — `reconcile.sh::reconcile_all` already enforces project_uuid scoping via `config::get_project_id`; verified by `bats tests/integration/us1-fresh-reconcile.bats`.
...
- [x] T063 [US4] [3] ✓ 2026-05-28 Update `src/install.sh` to detect Linear workspace seeded-state on first install — query `workflow_state_uuids` presence; if absent and operator has just resolved Team UUID, prompt to run `speckit.linear.seed` immediately or defer. Non-interactive mode halts with the FR-022 error rather than prompt; interactive defer surfaces an FR-022 warning row in the summary block. Covered by `tests/integration/us4-seed-prompt.bats`.
- [x] T064 [US4] [3] ✓ 2026-05-28 Update `src/install.sh` to interactively offer Action installation per FR-027 — opt-in prompt (default Y), copies `templates/github-action.yml` into `.github/workflows/spec-kit-linear-sync.yml`, prints `gh secret set LINEAR_API_TOKEN` instructions per FR-029. Idempotent overwrite-protection (interactive runs prompt before clobbering an operator-customised workflow); non-interactive runs install at canonical path without prompt; explicit `--no-action` suppresses the prompt. Missing template surfaces an error rather than silent skip. Covered by `tests/integration/us4-install-action.bats`.
- [x] T065 [US4] [1] ✓ 2026-05-28 `commands/linear-install.md`: documented the full install ceremony walkthrough end-to-end per `quickstart.md` (referencing operator-facing prose, not duplicating it). Added T063 seed-prompt + T064 Action-prompt sections to the algorithm, expanded the arguments / environment tables (FR-033b `SPECKIT_LINEAR_DOGFOOD_SAFE`, `--no-action`, `--no-prompt`), and extended the failure-surface section.
...
- [x] T081 [P] [1] Final shellcheck pass — zero warnings on every `src/*.sh`; document any intentional suppressions inline with justification ✓ 2026-05-28 — `shellcheck --shell=bash --severity=style --external-sources src/*.sh tests/perf/*.sh scripts/*.sh templates/git-hooks/*` clean, zero warnings. Verified repo-wide post-`src/pull.sh` landing.
- [x] T082 [3] Constitution Re-Check — walk through all 8 principles against the shipped implementation and confirm no drift between plan-time check and as-built. Update `validation/` with a post-implementation constitution-compliance report ✓ 2026-05-28 — landed in commit `b3b723c` as `validation/constitution-recheck-2026-05-28.md`. Verdict: 7 Conform / 1 caveat / 0 Drift. Cleared for merge.
